...
class DB extends MySQL
{
	...
	*/
	...
	{
		$this->query('select count(*) as cnt from twbz_pastebin where domain=?', $subdomain);
	...
	*/
	function _trimDomainPosts($subdomain, $deletecount)
	{
		...
		$sql='delete from twbz_pastebin where pid in (';
		$sep='';
		$this->query("select * from twbz_pastebin where domain=? order by posted asc limit $deletecount", $subdomain);
	...
	*/
	...
	{
		$this->query("delete from twbz_pastebin where expires is not null and now() > expires");	
	}
	...
	*/
	...
	{
		$this->query('delete from twbz_pastebin where pid=?', $pid);	
		return true;
	}
	...
	*/
	function addPost($poster,$subdomain,$format,$code,$parent_pid,$expiry_flag,$token)
	{
		...
		$this->query('insert into twbz_pastebin (poster, domain, posted, format, code, parent_pid, expires,expiry_flag) '.
				"values (?, ?, now(), ?, ?, ?, $expires, ?)",
		...
		$this->query('lock tables twbz_recent write');
		$this->query('update twbz_recent set seq_no=seq_no+1 where domain=? order by seq_no desc', $subdomain);
		$this->query('insert into twbz_recent (domain,seq_no,pid) values (?,1,?)', $subdomain, $id);
		$this->query('delete from twbz_recent where domain=? and seq_no=11', $subdomain);
		$this->query('unlock tables');
		
		//flush recent list
		$this->_cacheflush('twbz_recent'.$domain);		
	...
	*/
	function getPost($id, $subdomain)
	{
		$this->query('select *,date_format(posted, \'%a %D %b %H:%i\') as postdate '.
			'from twbz_pastebin where pid=? and domain=?', $id, $subdomain);
		if ($this->next_record())
			return $this->row;
		else
			return false;
	...
	*/
	function getRecentPostSummary($subdomain, $count)
	{
		...
		$posts=$this->_cachedquery($cacheid, "select p.pid,p.poster,unix_timestamp()-unix_timestamp(p.posted) as age, ".
			...
			"from twbz_pastebin as p ".
			"inner join twbz_recent as r on (r.domain=? and p.pid=r.pid) ".
	...
	*/
	function getFollowupPosts($pid, $limit=5)
	{
		...
		$this->query("select pid,poster,".
			...
			"from twbz_pastebin where parent_pid=? ".
	...
	*/
	...
	{
		$this->query("update twbz_pastebin set codefmt=?,codecss=? where pid=?",
			$codefmt, $codecss, $pid);
	}
